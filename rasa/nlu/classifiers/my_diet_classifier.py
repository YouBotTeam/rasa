...
from rasa.nlu.classifiers.diet_classifier import DIET
...
from rasa.nlu.classifiers.custom_classifier import Classifier
...
class My_Diet_Classifier(Classifier, EntityExtractor):
    ...
    def __init__(self,
        ...
        self.component_config = component_config if component_config is not None else PARAMS
    ...
    def preprocessing(self, training_data: TrainingData) -> RasaModelData:
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        model_data = self.preprocessing(training_data)
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        intent_dict, entities = None, None

        if self.component_config[INTENT_CLASSIFICATION]:
            label, label_ranking = self._predict_label(out)

            intent_dict = {'label': label,
    ...
    def load(
            ...
            cached_component: Optional["My_Diet_Classifier"] = None,
        ...
        model_meta = meta['params']
        model_meta['file'] = meta['file']
        model_meta = train_utils.update_confidence_type(model_meta)
        model_meta = train_utils.update_similarity_type(model_meta)
        model_meta = train_utils.update_deprecated_loss_type(model_meta)
    ...
    def _load_from_files(cls, meta: Dict[Text, Any], model_dir: Text):
        ...
        logger.debug('Loading data_example.pkl')
        data_example = io_utils.pickle_load(model_dir / f"{file_name}.data_example.pkl")

        logger.debug('Loading label_data.pkl')
        label_data = io_utils.pickle_load(model_dir / f"{file_name}.label_data.pkl")

        logger.debug('Loading label data')
        label_data = RasaModelData(data=label_data)

        logger.debug('Loading index_label_id_mapping.json')
        index_label_id_mapping = io_utils.json_unpickle(
            model_dir / f"{file_name}.index_label_id_mapping.json"
        )
        logger.debug('Loading entity_tag_specs.json')
    ...
    def _load_model(
        ...
        logger.debug('Loading RasaModel')
    ...
    def intent_to_rasa_output(self, intent):

        if intent is None:
            return None

        result = {'value': intent['label'], # {"name": None, "id": None, "confidence": 0.0}
                  'ranking': intent['ranking'],
                  'extractor': 'My_Diet_Classifier'}

        return result

    def entities_to_rasa_output(self, entities):
        if (entities is None):
            return None

        return entities

    def pred_to_rasa_output(self, intent, entities):
        """
        ...
        rasa_output = {INTENT: self.intent_to_rasa_output(intent),
                       ENTITIES: self.entities_to_rasa_output(entities)}
